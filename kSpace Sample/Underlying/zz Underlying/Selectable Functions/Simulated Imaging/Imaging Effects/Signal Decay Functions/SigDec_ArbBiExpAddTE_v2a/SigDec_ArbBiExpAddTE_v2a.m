...
classdef SigDec_ArbBiExpAddTE_v2a < handle

properties (SetAccess = private)                   
    Method = 'SigDec_ArbBiExpAddTE_v2a'
    T2f,T2s,TE,FracT2f
    Panel = cell(0);
    NormToOne
end
...
% Constructor
%==================================================================  
function [SIGDEC,err] = SigDec_ArbBiExpAddTE_v2a(SIGDECipt)    
    err.flag = 0;
...
function [SampDat,err] = AddPostEffect(SIGDEC,SIMMETH,STCH,SampDat0)
    ...
    T2decay = SIGDEC.FracT2f*exp(-(STCH.SamplingTimeOnTrajectory+SIGDEC.TE)/SIGDEC.T2f) + (1-SIGDEC.FracT2f)*exp(-(STCH.SamplingTimeOnTrajectory+SIGDEC.TE)/SIGDEC.T2s);
