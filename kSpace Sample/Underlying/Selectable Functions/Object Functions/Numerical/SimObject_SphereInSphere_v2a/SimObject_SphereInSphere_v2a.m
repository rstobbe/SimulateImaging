...
classdef SimObject_SphereInSphere_v2a < handle

properties (SetAccess = private)                   
    Method = 'SimObject_SphereInSphere_v2a'
    SphereDiam
    InnerSphereDiam 
    InnerSphereIntensity    
    RelDiam
    InnerRelDiam
    ObMatSz
    SimOb
...
% Constructor
%==================================================================  
function [SIMOB,err] = SimObject_SphereInSphere_v2a(SIMOBipt)    
    err.flag = 0;
    SIMOB.SphereDiam = str2double(SIMOBipt.('SphereDiam'));
    SIMOB.InnerSphereDiam = str2double(SIMOBipt.('InnerSphereDiam'));
    SIMOB.InnerSphereIntensity = str2double(SIMOBipt.('InnerSphereIntensity'));
end
...
function err = BuildSimObject(SIMOB,SIMMETH,WRT)
    ...
    SIMOB.InnerRelDiam = (SIMOB.InnerSphereDiam/WRT.STCH{1}.Fov)/SIMMETH.KSMP.IF.SS;
    ...
    for x = 1:M
        for y = 1:M
            for z = 1:M
                r = sqrt((x-CX)^2 + (y-CY)^2 + (z-CZ)^2);
                if r <= rmax
                    SIMOB.SimOb(z,x,y) = SIMOB.InnerSphereIntensity;
                end
            end
        end
    end    
    ...
    %---------------------------------------------
    SIMOB.ObMatSz = M;
    SIMOB.name = ['Sph',num2str(SIMOB.SphereDiam),'Sph',num2str(SIMOB.InnerSphereDiam)];
    ...
    SIMOB.Panel(4,:) = {'InnerSphereDiam (mm)',SIMOB.InnerSphereDiam,'Output'};
    SIMOB.Panel(5,:) = {'InnerSphereIntensity (mm)',SIMOB.InnerSphereIntensity,'Output'};
